...
const ProductCard: React.FC<{ product: any; isDark: boolean; className?: string }> = ({ product, isDark, className = '' }) => {
  ...
  return (
    <Link to={`/product/${product.id}`} className={`group relative flex flex-col ${className}`}>
      <div className={`relative rounded-none overflow-hidden border transition-all duration-300 hover:shadow-[0_4px_20px_rgb(0,0,0,0.08)] ${isDark ? 'bg-[#0a0a0a] border-white/[0.03] hover:border-yellow-400/30' : 'bg-white border-slate-100 hover:border-yellow-400'}`}>
        ...
        <div className="relative aspect-square overflow-hidden bg-slate-50 dark:bg-zinc-950">
          ...
          <button
            ...
            className="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center transition-transform active:scale-90"
          >
        ...
        <div className="p-2 space-y-1">
          ...
          <div className="flex items-center justify-between pt-1 border-t border-dashed border-white/5">
            ...
            <button 
              onClick={handleAdd}
              className={`px-2 py-1 rounded-none text-[8px] font-black uppercase tracking-widest transition-all ${
                added 
    ...
    <div className={`max-w-[1920px] mx-auto flex flex-col h-[calc(100vh-140px)] overflow-hidden border ${borderClass} ${bgClass} shadow-2xl relative`}>
      ...
      <div className={`px-6 py-2 border-b ${borderClass} ${isDark ? 'bg-[#050505]' : 'bg-white'} flex items-center justify-between z-30`}>
        ...
        <div className="flex items-center gap-2">
          {/* Top Bar Search */}
          <div className={`relative w-48 flex items-center px-3 py-1.5 rounded-none border ${isDark ? 'bg-white/[0.03] border-white/5 focus-within:border-yellow-400/50' : 'bg-slate-50 border-slate-100 focus-within:border-yellow-400'} transition-all`}>
        ...
        <aside className={`w-[200px] flex-shrink-0 border-r ${borderClass} ${sidebarBg} overflow-y-auto hidden lg:flex flex-col`}>
          <nav className="p-3 pt-5 flex-1 space-y-1">
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-none transition-all text-left group ${
                !selectedCategory 
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-1.5 rounded-none transition-all text-left group ${
                    isActive 
                      ? 'bg-yellow-400 text-black shadow shadow-yellow-400/10' 
                      : `${isDark ? 'text-slate-500 hover:bg-white/5 hover:text-white' : 'text-slate-600 hover:bg-white hover:text-slate-900'}`
                  }`}
                >
                  <div className={`w-6 h-6 rounded-none flex items-center justify-center shrink-0 transition-all ${
          ...
          <div className={`flex-1 overflow-y-auto p-4 custom-scrollbar ${isDark ? 'bg-[#0a0a0a]' : 'bg-slate-50/30'}`}>
            {filteredProducts.length > 0 ? (
              ...
              <div className="h-full flex flex-col items-center justify-center gap-4 py-20">
                <div className={`w-12 h-12 rounded-none ${isDark ? 'bg-white/5' : 'bg-white'} border ${borderClass} flex items-center justify-center animate-pulse`}>
                ...
                <div className="text-center space-y-1">
                  ...
                  <button 
                    ...
                    className="mt-4 px-5 py-2 bg-yellow-400 text-black rounded-none font-black text-[8px] uppercase tracking-widest hover:bg-yellow-300"
                  >
                    Reset
                  </button>
                </div>
