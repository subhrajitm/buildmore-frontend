...
import { Plus, Pencil, Trash2, Package, ToggleLeft, ToggleRight, Check, X, Upload, AlertCircle, ChevronDown } from 'lucide-react';
...
export const Admin: React.FC<AdminProps> = ({ isDark }) => {
  ...
  const handleUpdate = async (id: string) => {
    if (!token) return;
    try {
      await adminApi.update(id, editForm, token);
      setEditingId(null);
  ...
  const handleStockUpdate = async (id: string) => {
    ...
    try {
      await adminApi.updateStock(id, n, token);
  ...
  const handleToggleAvailability = async (id: string) => {
    if (!token) return;
    try {
      await adminApi.toggleAvailability(id, token);
      showToast('Availability toggled');
      loadProducts();
    } catch (err: any) {
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-white/5 pb-8">
          <div className="space-y-1">
            ...
            <h1 className={`text-4xl font-black tracking-tighter ${isDark ? 'text-white' : 'text-slate-900'}`}>
              Product <span className="text-yellow-400">Control</span>
            </h1>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{products.length} Products in Catalog</p>
          </div>
          <button
            onClick={() => { setShowAddForm(v => !v); setError(''); }}
            className="flex items-center gap-3 bg-yellow-400 text-black px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-yellow-300 transition-all shadow-lg shadow-yellow-400/20"
          >
            ...
            <form onSubmit={handleAdd} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 rounded-xl border text-xs font-bold outline-none transition-colors ${input}`}
                    placeholder="BuildMore X-Series Drill..."
                  />
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className={`w-full px-4 py-3 rounded-xl border text-xs font-bold outline-none transition-colors resize-none ${input}`}
                  placeholder="Professional-grade industrial component..."
                />
              </div>
              <div className="space-y-2">
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 rounded-xl border text-xs font-bold outline-none transition-colors ${input}`}
                  placeholder="Composite/Aluminum, IP67 Certified..."
                />
              </div>
              <div className="space-y-2">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-500">Product Images * (up to 5)</label>
                <div
                  className={`border-2 border-dashed rounded-xl p-6 flex flex-col items-center gap-3 cursor-pointer transition-colors ${isDark ? 'border-white/10 hover:border-yellow-400/40' : 'border-slate-200 hover:border-yellow-400/60'}`}
              ...
              <div className="flex gap-4 pt-2">
                <button
                  type="submit"
                  disabled={submitting}
                  className="bg-yellow-400 text-black px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-yellow-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  {submitting ? <><div className="w-4 h-4 border-2 border-black border-t-transparent rounded-full animate-spin" /> Saving...</> : <><Check className="w-4 h-4" /> Save Product</>}
                </button>
                <button
                  type="button"
                  onClick={() => { setShowAddForm(false); setError(''); setForm({ ...EMPTY_FORM }); }}
                  className={`px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${isDark ? 'bg-white/5 text-slate-400 hover:bg-white/10' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                >
                  Cancel
                </button>
              </div>
                      ...
                      {editingId === p._id ? (
                        <>
                          <td className="px-6 py-4" colSpan={4}>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                              <input
                                value={editForm.productName ?? p.productName}
                                onChange={e => setEditForm(f => ({ ...f, productName: e.target.value }))}
                                className={`px-3 py-2 rounded-lg border text-xs font-bold outline-none ${input}`}
                                placeholder="Name"
                              />
                              <input
                              ...
                              />
                              <input
                                value={editForm.materialSpecifications ?? p.materialSpecifications ?? ''}
                                onChange={e => setEditForm(f => ({ ...f, materialSpecifications: e.target.value }))}
                                className={`px-3 py-2 rounded-lg border text-xs font-bold outline-none ${input}`}
                          ...
                          <td className="text-right px-6 py-4">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                ...
                                className="bg-yellow-400 text-black px-4 py-2 rounded-lg font-black text-[9px] uppercase tracking-widest hover:bg-yellow-300 transition-all flex items-center gap-1.5"
                              >
                                <Check className="w-3 h-3" /> Save
                              </button>
                              <button
                                onClick={() => { setEditingId(null); setEditForm({}); }}
                                className={`px-4 py-2 rounded-lg font-black text-[9px] uppercase tracking-widest transition-all ${isDark ? 'bg-white/5 text-slate-400 hover:bg-white/10' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                              >
                                <X className="w-3 h-3" />
                              </button>
                            </div>
                        ...
                        <>
                          ...
                          <td className="px-4 py-4">
                            <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded ${isDark ? 'bg-white/5 text-slate-400' : 'bg-slate-100 text-slate-600'}`}>{p.category}</span>
                          </td>
                          <td className="px-4 py-4 text-right">
                            <span className={`text-sm font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>${p.price.toFixed(2)}</span>
                          </td>
                          <td className="px-4 py-4 text-right">
                            {stockEditing === p._id ? (
                              <div className="flex items-center justify-end gap-2">
                                <input
                                  ...
                                  className={`w-20 px-2 py-1 rounded-lg border text-xs font-bold outline-none text-right ${input}`}
                                  autoFocus
                                />
                                <button onClick={() => handleStockUpdate(p._id)} className="text-yellow-400 hover:text-yellow-300">
                                  <Check className="w-3.5 h-3.5" />
                                </button>
                                <button onClick={() => setStockEditing(null)} className="text-slate-500 hover:text-slate-300">
                                  <X className="w-3.5 h-3.5" />
                                </button>
                              </div>
                            ) : (
                              <button
                                onClick={() => { setStockEditing(p._id); setStockValue(String(p.stock)); }}
                                className={`text-xs font-black hover:text-yellow-400 transition-colors flex items-center gap-1 ml-auto ${p.stock === 0 ? 'text-red-400' : isDark ? 'text-white' : 'text-slate-900'}`}
                              >
                          ...
                          <td className="px-4 py-4 text-center">
                            <button
                              ...
                              title={p.availability ? 'Click to disable' : 'Click to enable'}
                            >
                              {p.availability
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                ...
                                className={`p-2 rounded-lg transition-all ${isDark ? 'hover:bg-white/5 text-slate-400 hover:text-yellow-400' : 'hover:bg-slate-100 text-slate-500 hover:text-yellow-500'}`}
                                title="Edit product"
                              >
                              ...
                              <button
                                ...
                                className={`p-2 rounded-lg transition-all ${isDark ? 'hover:bg-red-500/10 text-slate-400 hover:text-red-400' : 'hover:bg-red-50 text-slate-500 hover:text-red-500'}`}
