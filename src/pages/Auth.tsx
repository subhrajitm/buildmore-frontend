...
import { BarChart3, Mail, Lock, ArrowRight, ShieldCheck, Globe } from 'lucide-react';
    ...
    <div className="min-h-[80vh] flex items-center justify-center animate-in zoom-in-95 duration-500">
      <div className={`w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 rounded-3xl overflow-hidden border ${isDark ? 'bg-zinc-900 border-white/5 shadow-2xl' : 'bg-white border-slate-100 shadow-xl'}`}>
        ...
        <div className="p-12 space-y-10">
          ...
          <div className="space-y-4">
            <button className="w-full bg-yellow-400 text-black py-5 rounded-xl font-black text-xs uppercase tracking-[0.2em] hover:bg-yellow-300 transition-all shadow-2xl flex items-center justify-center gap-3">
              Authorize Entrance <ArrowRight className="w-4 h-4" />
            </button>
            <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-widest">
               <button className="text-slate-500 hover:text-yellow-400 transition-colors">Request ID Reset</button>
               <button className="text-slate-500 hover:text-yellow-400 transition-colors">Apply for Enterprise Account</button>
            </div>
          </div>
