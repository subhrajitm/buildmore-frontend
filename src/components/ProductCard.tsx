...
export const ProductCard: React.FC<ProductCardProps> = ({ product, isDark }) => (
  <div className={`rounded-xl overflow-hidden flex flex-col border transition-all group shadow-sm ${isDark ? 'bg-zinc-900 border-white/5 hover:border-yellow-400/20' : 'bg-white border-slate-100 hover:border-yellow-400/40 hover:shadow-xl'}`}>
    <div className={`relative h-48 flex items-center justify-center p-6 transition-colors duration-300 ${isDark ? 'bg-white/5' : 'bg-slate-50'}`}>
      ...
      />
      {product.discount && (
        <span className="absolute top-3 left-3 bg-yellow-400 text-black text-[8px] font-black px-2 py-1 rounded-sm uppercase tracking-widest">
          {product.discount}
        </span>
      )}
    ...
    <div className="p-4 flex-1 flex flex-col space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">ID: {product.id}</span>
      ...
      <Link to={`/product/${product.id}`}>
        <h3 className={`font-black text-xs leading-tight line-clamp-2 h-8 uppercase hover:text-yellow-400 transition-colors ${isDark ? 'text-white' : 'text-slate-900'}`}>
          {product.name}
        </h3>
      </Link>
      <div className="pt-1">
        ...
        {product.bulkInfo && (
          <div className={`${isDark ? 'bg-white/5 text-slate-400' : 'bg-slate-100 text-slate-500'} text-[8px] font-black px-2 py-0.5 rounded-sm inline-block mt-2 uppercase tracking-widest`}>
            {product.bulkInfo}
          </div>
        )}
      </div>
      <div className="pt-4 mt-auto">
        <button className="w-full bg-yellow-400 text-black text-[9px] font-black py-3 rounded-lg uppercase tracking-widest hover:bg-yellow-300 transition-all flex items-center justify-center gap-2 shadow-sm">
          {product.isTechnical ? 'Specifications' : 'Add to Project'}
