    ...
    <div className={`rounded-xl overflow-hidden flex flex-col border transition-all group shadow-sm ${isDark ? 'bg-zinc-900 border-white/5 hover:border-yellow-400/20' : 'bg-white border-slate-100 hover:border-yellow-400/40 hover:shadow-xl'}`}>
      ...
      <div className="p-4 flex-1 flex flex-col space-y-2">
        ...
        <div className="pt-4 mt-auto">
          <button
            onClick={handleAdd}
            className={`w-full text-[9px] font-black py-3 rounded-lg uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-sm ${
              added
          ...
          >
            {added ? (
              ...
              <>{product.isTechnical ? 'Specifications' : 'Add to Project'}<ArrowRight className="w-3 h-3" /></>
            )}
          </button>
        </div>
      </div>
